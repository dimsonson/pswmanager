...
func (h *ClientRMQhandlers) PublishLogins(ctx context.Context, in *pbpub.PublishLoginsRequest) (*pbpub.PublishLoginsResponse, error) {
	...
	err := h.ClientRMQSvs.PublishRecord(ctx, in.ExchName, in.RoutingKey, loginsRecord)
	if err != nil {
		log.Printf("call PublishRecord error: %v", err)
		err = status.Errorf(codes.Internal, `server error %s`, error.Error(err))
		if err != nil {
...
func (h *ClientRMQhandlers) PublishBinary(ctx context.Context, in *pbpub.PublishBinaryRequest) (*pbpub.PublishBinaryResponse, error) {
	...
	err := h.ClientRMQSvs.PublishRecord(ctx, in.ExchName, in.RoutingKey, binaryRecord)
	if err != nil {
		log.Printf("call PublishRecord error: %v", err)
		err = status.Errorf(codes.Internal, `server error %s`, error.Error(err))
		if err != nil {
...
func (h *ClientRMQhandlers) PublishCard(ctx context.Context, in *pbpub.PublishCardRequest) (*pbpub.PublishCardResponse, error) {
	...
	err := h.ClientRMQSvs.PublishRecord(ctx, in.ExchName, in.RoutingKey, cardRecord)
	if err != nil {
		log.Printf("call PublishRecord error: %v", err)
		err = status.Errorf(codes.Internal, `server error %s`, error.Error(err))
		if err != nil {
